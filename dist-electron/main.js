...
const getGoogleFinanceQuote = async (stockCode) => {
  var _a;
  const code = stockCode.padStart(4, "0");
  const url2 = `https://www.google.com/finance/quote/${code}:HKG`;
  ...
  const clean2 = (s) => s.replace(/[$,\s]/g, "");
  const pick = (re) => {
    const m = html.match(re);
    return m ? clean2(m[1]) : "";
  };
  const name = ((_a = (html.match(/<title>([^(<]+)/) ?? [])[1]) == null ? void 0 : _a.trim()) ?? code;
  const price = pick(/data-last-price="([0-9,.]+)"/) || pick(/class="YMlKec[^"]*">\$?([0-9,.]+)<\/div>/) || pick(/([0-9,]+\.[0-9]{2,3})[^0-9.]{1,60}HKD/) || pick(/([0-9,]+\.[0-9]{2,3})[^0-9.]{1,60}HKG/);
  if (!price) throw new Error(`Google Finance: price not found for ${code} (${url2})`);
  const prevClose = pick(/Previous close[^0-9]*([0-9,.]+)/);
  const dayM = html.match(/Day range[^0-9]*([0-9,.]+)[^0-9,.]+([0-9,.]+)/);
  const low = dayM ? clean2(dayM[1]) : "-";
  const high = dayM ? clean2(dayM[2]) : "-";
  const yrM = html.match(/Year range[^0-9]*([0-9,.]+)[^0-9,.]+([0-9,.]+)/);
  const lo52 = yrM ? clean2(yrM[1]) : "-";
  const hi52 = yrM ? clean2(yrM[2]) : "-";
  return { name, price, high, low, prevClose, hi52, lo52 };
};
const getYahooChart = async (stockCode) => {
  var _a, _b, _c, _d, _e, _f;
  const symbol = `${stockCode.padStart(4, "0")}.HK`;
  const [r1y, r5d] = await Promise.all([
    axios.get(`https://query1.finance.yahoo.com/v8/finance/chart/${symbol}?range=1y&interval=1d`, { timeout: 8e3, headers: YAHOO_HEADERS }),
    axios.get(`https://query1.finance.yahoo.com/v8/finance/chart/${symbol}?range=5d&interval=1d`, { timeout: 8e3, headers: YAHOO_HEADERS })
  ]);
  const result = (_a = r1y.data.chart.result) == null ? void 0 : _a[0];
  if (!result) throw new Error("Yahoo chart data not found");
  const q = result.indicators.quote[0];
  const ts = result.timestamp ?? [];
  const history = ts.map((t, i) => ({ date: new Date(t * 1e3).toISOString().slice(0, 10), close: q.close[i] })).filter((p) => Number.isFinite(p.close));
  const closes5d = (((_f = (_e = (_d = (_c = (_b = r5d.data.chart.result) == null ? void 0 : _b[0]) == null ? void 0 : _c.indicators) == null ? void 0 : _d.quote) == null ? void 0 : _e[0]) == null ? void 0 : _f.close) ?? []).filter(Number.isFinite);
  return {
...
const getGoogleFinanceStock = async (stockCode, sourceName) => {
  const code = stockCode.padStart(4, "0");
  ...
  const prevClose = (chart == null ? void 0 : chart.previousClose) !== null && (chart == null ? void 0 : chart.previousClose) !== void 0 ? formatNumber(chart.previousClose) : gf.prevClose || "-";
  return {
    source: sourceName,
    name: gf.name || code,
    number: code,
    ...
    history: (chart == null ? void 0 : chart.history) ?? []
  };
};
const getYahooStock = async (stockCode) => {
  var _a, _b, _c, _d, _e, _f;
  const code = stockCode.padStart(4, "0");
  ...
  const result = (_a = response.data.chart.result) == null ? void 0 : _a[0];
  if (!result) {
  ...
  const history = timestamps.map((timestamp, index) => {
    ...
    return {
      date: new Date(timestamp * 1e3).toISOString().slice(0, 10),
  ...
  const closes5d = (((_f = (_e = (_d = (_c = (_b = response5d.data.chart.result) == null ? void 0 : _b[0]) == null ? void 0 : _c.indicators) == null ? void 0 : _d.quote) == null ? void 0 : _e[0]) == null ? void 0 : _f.close) ?? []).filter(Number.isFinite);
...
const getEastmoneyStock = async (stockCode) => {
  var _a;
  const code = stockCode.padStart(4, "0");
  ...
  return {
    ...
    previousClose: (chart == null ? void 0 : chart.previousClose) != null ? formatNumber(chart.previousClose) : formatNumber(d.f60),
    avg10: formatNumber(average(closes, 10)),
    avg20: formatNumber(average(closes, 20)),
    avg50: formatNumber(average(closes, 50)),
    avg100: formatNumber(average(closes, 100)),
...
app.whenReady().then(() => {
  ...
  ipcMain.handle("get-stock", async (_event, stockCode, source = "auto") => {
