...
const getEtnetQuote = (stockCode) => {
  ...
  return new Promise((resolve2, reject) => {
    ...
    const timer = setTimeout(
      () => settle2(() => reject(new Error("ETNet timeout"))),
      2e4
    );
    win.loadURL(
    ...
    win.webContents.on("did-fail-load", (_e, _code, desc) => {
      clearTimeout(timer);
      settle2(() => reject(new Error(`ETNet load failed: ${desc}`)));
    });
  });
};
const getYahooStock = async (stockCode) => {
  ...
  const previousClose = meta.regularMarketPreviousClose ?? meta.chartPreviousClose ?? meta.previousClose ?? (closes.length > 0 ? closes[closes.length - 1] : null);
  return {
    source: "雅虎財經",
...
app.whenReady().then(() => {
  ipcMain.handle("get-stock", async (_event, stockCode) => {
